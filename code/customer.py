...
class Cust_Win:
    def __init__(self,root):
        ...
        #title
        lbl_title=Label(self.root,text="ADD CUSTOMER DETAILS",font=("impact",18),bg="black",fg="white",relief=RIDGE)
        ...
        #label frame
        labelframeleft=LabelFrame(self.root,bd=2,relief=RIDGE,text="Customer Details",font=("impact",12),padx=2)
        labelframeleft.place(x=5,y=50,width=425,height=490)

        #labels and entries
        #entry reference
        lbl_cust_ref=Label(labelframeleft,text="Customer Reference ",font=("impact",12),padx=2,pady=6)
        lbl_cust_ref.grid(row=0,column=0,sticky=W)

        entry_ref=ttk.Entry(labelframeleft,textvariable=self.var_ref,width=29,font=("impact",13),state="readonly")
        entry_ref.grid(row=0,column=1)

        # cust name
        cname=Label (labelframeleft, font=("impact", 12,), text="Customer Name", padx=2,pady=6)
        cname. grid (row=1, column=0, sticky=W)
        txtcname=ttk. Entry(labelframeleft,textvariable=self.var_cust_name, font=("arial",13, "bold"), width=29)
        txtcname. grid(row=1, column=1)

        # mother name
        lblmname=Label(labelframeleft, font=("impact",12), text="Mother's Name " , padx=2, pady=6)
        lblmname.grid(row=2, column=0, sticky=W)
        txtmname=ttk.Entry(labelframeleft,textvariable=self.var_mother, font=("arial",13, "bold"),width=29)
        txtmname.grid (row=2, column=1)

        # gender combobox
        label_gender=Label(labelframeleft, font=("impact",12), text="Gender ", padx=2, pady=6)
        ...
        lblIdProof=Label(labelframeleft, font=("impact",12), text="Id Proof Type ", padx=2, pady=6)
        ...
        combo_idProof["value"]=("Aadhar/SSN","Driving License","Passport")
        combo_idProof.grid(row=8,column=1)

        #id number
        lblIdNumber=Label (labelframeleft, font=("impact",12), text="Id Number ", padx=2, pady=6)
        lblIdNumber.grid(row=9, column=0, sticky=W)
        txtIdNumber=ttk. Entry (labelframeleft,textvariable=self.var_id_number, font=("arial", 13, "bold"), width=29)
        txtIdNumber.grid(row=9, column=1)
        
        # address
        lblAddress=Label (labelframeleft, font=("impact",12), text="Address ", padx=2, pady=6)  
        ...
        btnDelete=Button(btn_frame,command=self.mDelete,text="Delete",font=("impact",12),bg="black",fg="white",width=10)
        btnDelete.grid(row=0,column=2,padx=1)

        btnReset=Button(btn_frame,command=self.reset,text="Reset",font=("impact",12),bg="black",fg="white",width=10)
        ...
        combo_Search["value"]=("Mobile Number","Customer Reference")
        ...
        btnSearch=Button(Table_Frame,text="Search",command=self.search,font=("impact",12),bg="black",fg="white",width=10)
        btnSearch.grid(row=0,column=3,padx=1)

        btnShowAll=Button(Table_Frame,text="Show All",command=self.fetch_data,font=("impact",12),bg="black",fg="white",width=10)
        ...
        self.Cust_Details_Table=ttk.Treeview(details_table,columns=("Customer Reference","Customer Name","Mother's Name","Gender","Postal Code","Mobile Number","Email","Nationality","Id Proof Type","Id Number","Address"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
        ...
        self.Cust_Details_Table.heading("Customer Reference",text="Customer Reference")
        ...
        """
        self.Cust_Details_Table.column("Customer Reference",width=120)
    ...
    def update(self):
        if self.var_mobile.get()=="":
            ...
            conn=mysql.connector.connect(host="localhost",username="root",password="ambagelat0",database="dbmspackage")
            my_cursor=conn.cursor()
            my_cursor.execute("update customer set 'Customer Name'=%s, 'Mother's Name'=%s, 'Gender'=%s,'Postal Code'=%s,' Mobile Number'=%s,' Email'=%s,' Nationality'=%s,' Id Proof Type'=%s,' Id Number'=%s,' Address'=%s where' Customer Reference'=%s",
                                  self.var_cust_name.get(),self.var_mother.get(),self.var_gender.get(),self.var_post.get(),
                                  self.var_mobile.get(),self.var_email.get(),self.var_nationality.get(),self.var_id_proof.get(),
    ...
    def update(self):
        if self.var_mobile.get() == "":
            ...
            sql_query = """
            UPDATE customer 
            SET `Customer Name`=%s, `Mother's Name`=%s, Gender=%s, `Postal Code`=%s, `Mobile Number`=%s, 
                Email=%s, Nationality=%s, `Id Proof Type`=%s, `Id Number`=%s, Address=%s 
            WHERE `Customer Reference`=%s
            """
    ...
    def mDelete(self):
        ...
        if mDelete>0:
            ...
            my_cursor.execute(query, (self.var_ref.get(),))
    ...
    def search(self):
        ...
        search_query_mobile = "SELECT * FROM customer WHERE `Mobile Number` LIKE %s"
        search_query_reference = "SELECT * FROM customer WHERE `Customer Reference` LIKE %s"
    
        search_term = '%' + self.txt_search.get() + '%'  # Construct search term
    
        my_cursor.execute(search_query_mobile, (search_term,))
        rows_mobile = my_cursor.fetchall()
        ...
        my_cursor.execute(search_query_reference, (search_term,))
        rows_reference = my_cursor.fetchall()
    
        # Combine results from both queries
        rows = rows_mobile + rows_reference
